...
7. WORK_ITEM / RECURRING_RULE 의 `owner_node_title` 은 소속 노드 이름이며, 노드를 찾지 못하면 null 입니다.<br>
8. 사용자가 존재하지 않으면 404와 `요청자를 찾을 수 없습니다.` 메시지가, 조회 중 오류가 나면 500과 `대시보드 데이터를 불러오지 못했습니다.` 메시지가 반환됩니다.<br>
