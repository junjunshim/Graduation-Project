...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        INSERT INTO work_item_histories (work_item_id, display_id, owner_node_id, parent_work_item_id, owner_user_id, title, description, category, hidden, status, priority, weight, progress, is_deleted, start_date, due_date, created_at, updated_at, change_status)
        VALUES (OLD.work_item_id, OLD.display_id, OLD.owner_node_id, OLD.parent_work_item_id, OLD.owner_user_id, OLD.title, OLD.description, OLD.category, OLD.hidden, OLD.status, OLD.priority, OLD.weight, OLD.progress, OLD.is_deleted, OLD.start_date, OLD.due_date, OLD.created_at, OLD.updated_at, 'deleted');
    ELSIF (TG_OP = 'UPDATE') THEN
        INSERT INTO work_item_histories (work_item_id, display_id, owner_node_id, parent_work_item_id, owner_user_id, title, description, category, hidden, status, priority, weight, progress, is_deleted, start_date, due_date, created_at, updated_at, change_status)
        VALUES (NEW.work_item_id, NEW.display_id, NEW.owner_node_id, NEW.parent_work_item_id, NEW.owner_user_id, NEW.title, NEW.description, NEW.category, NEW.hidden, NEW.status, NEW.priority, NEW.weight, NEW.progress, NEW.is_deleted, NEW.start_date, NEW.due_date, NEW.created_at, NEW.updated_at, 'updated');
    ELSIF (TG_OP = 'INSERT') THEN
        INSERT INTO work_item_histories (work_item_id, display_id, owner_node_id, parent_work_item_id, owner_user_id, title, description, category, hidden, status, priority, weight, progress, is_deleted, start_date, due_date, created_at, updated_at, change_status)
        VALUES (NEW.work_item_id, NEW.display_id, NEW.owner_node_id, NEW.parent_work_item_id, NEW.owner_user_id, NEW.title, NEW.description, NEW.category, NEW.hidden, NEW.status, NEW.priority, NEW.weight, NEW.progress, NEW.is_deleted, NEW.start_date, NEW.due_date, NEW.created_at, NEW.updated_at, 'inserted');
    END IF;
    RETURN NULL;
END;
$$ LANGUAGE plpgsql;