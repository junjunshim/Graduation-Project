...
import api from './api';
import { GradNodeProvider, NodeItem } from './nodeProvider';
import { LoginViewProvider } from './loginViewProvider';
import { MemberViewProvider } from './memberViewProvider';
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    const memberProvider = new MemberViewProvider(context.extensionUri);
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider(MemberViewProvider.viewType, memberProvider)
    );
    ...
    // 2. 트리 클릭 시 실행될 커맨드 수정
    context.subscriptions.push(
        vscode.commands.registerCommand('grad-vscode.showMembers', (title: string, targetNodeId: string, isWorkItem: boolean) => {
            const members = gradNodeProvider.getMembersForNode(targetNodeId);
            
            const displayTitle = isWorkItem ? `${title} (Department Members)` : title;

            if (members.length === 0) {
                vscode.window.showInformationMessage(`${displayTitle}: 등록된 멤버가 없습니다.`);
                memberProvider.updateMemberList(displayTitle, []);
                return;
            }

            memberProvider.updateMemberList(displayTitle, members);
        })
    );
