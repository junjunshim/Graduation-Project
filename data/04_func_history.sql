...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        INSERT INTO organization_node_histories (node_id, node_type, parent_node_id, name, path, created_at, updated_at, change_status)
        VALUES (OLD.node_id, OLD.node_type, OLD.parent_node_id, OLD.name, OLD.path, OLD.created_at, OLD.updated_at, 'deleted');
        RETURN OLD;
    ELSIF (TG_OP = 'UPDATE') THEN
        INSERT INTO organization_node_histories (node_id, node_type, parent_node_id, name, path, created_at, updated_at, change_status)
        VALUES (NEW.node_id, NEW.node_type, NEW.parent_node_id, NEW.name, NEW.path, NEW.created_at, NEW.updated_at, 'updated');
        RETURN NEW;
    ELSIF (TG_OP = 'INSERT') THEN
        INSERT INTO organization_node_histories (node_id, node_type, parent_node_id, name, path, created_at, updated_at, change_status)
        VALUES (NEW.node_id, NEW.node_type, NEW.parent_node_id, NEW.name, NEW.path, NEW.created_at, NEW.updated_at, 'inserted');
        RETURN NEW;
    END IF;
    RETURN NULL;
END;
...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        INSERT INTO role_assignment_histories (user_id, node_id, role, created_at, updated_at, change_status)
        VALUES (OLD.user_id, OLD.node_id, OLD.role, OLD.created_at, OLD.updated_at, 'deleted');
        RETURN OLD;
    ELSIF (TG_OP = 'UPDATE') THEN
        INSERT INTO role_assignment_histories (user_id, node_id, role, created_at, updated_at, change_status)
        VALUES (NEW.user_id, NEW.node_id, NEW.role, NEW.created_at, NEW.updated_at, 'updated');
        RETURN NEW;
    ELSIF (TG_OP = 'INSERT') THEN
        INSERT INTO role_assignment_histories (user_id, node_id, role, created_at, updated_at, change_status)
        VALUES (NEW.user_id, NEW.node_id, NEW.role, NEW.created_at, NEW.updated_at, 'inserted');
        RETURN NEW;
    END IF;
    RETURN NULL;
END;
...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        INSERT INTO role_authority_histories (node_id, role, authority, created_at, updated_at, change_status)
        VALUES (OLD.node_id, OLD.role, OLD.authority, OLD.created_at, OLD.updated_at, 'deleted');
        RETURN OLD;
    ELSIF (TG_OP = 'UPDATE') THEN
        INSERT INTO role_authority_histories (node_id, role, authority, created_at, updated_at, change_status)
        VALUES (NEW.node_id, NEW.role, NEW.authority, NEW.created_at, NEW.updated_at, 'updated');
        RETURN NEW;
    ELSIF (TG_OP = 'INSERT') THEN
        INSERT INTO role_authority_histories (node_id, role, authority, created_at, updated_at, change_status)
        VALUES (NEW.node_id, NEW.role, NEW.authority, NEW.created_at, NEW.updated_at, 'inserted');
        RETURN NEW;
    END IF;
    RETURN NULL;
END;
