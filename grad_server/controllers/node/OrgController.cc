...
// 최근 활동 내역(이력 피드) 조회 API
void OrgController::getActivities(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback){
    // 1. 데이터 파싱 및 유효성 검사
    // 쿼리 파라미터에서 node_id와 target_email 가져오기
    std::string node_id_str = req->getParameter("node_id");
    std::string target_email = req->getParameter("target_email");

    // 선택 파라미터
    auto getStrOrNull = [&](const std::string &val) {
        return val.empty() ? "" : val;
    };
    auto getIntOrNull = [&](const std::string &val, int defaultVal) {
    ...
    // DB 함수 호출 SQL
    std::string sql = "SELECT * from get_activities($1, $2, $3)";

    // 헬퍼 함수를 통해 안전하게 매핑 수행
    int node_id = getIntOrNull(node_id_str, -1);
    std::string email_val = getStrOrNull(target_email);

    // node_id_str가 비어있지 않은데 숫자가 아닌 경우에 대한 유효성 예외 검증
    if(!node_id_str.empty() && node_id == -1){
        Json::Value ret;
        ret["status"] = "error";
        ret["code"] = "400";
    ...
    dbClient->execSqlAsync(
        ...
        // DB 함수에 전달할 매개변수 (요청자 이메일, 노드 ID, 대상 이메일)
        requester_email,
        node_id,
        email_val
    );
